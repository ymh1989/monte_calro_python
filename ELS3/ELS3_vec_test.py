...
S0_1 = 100.0;
S0_2 = 100.0; 
S0_3 = 100.0; # underlying price 1, 2, 3 
r1 = 0.03;
r2 = 0.03; 
r3 = 0.03; # riskless interest rate 1, 2, 3
sig1 = 0.3;
sig2 = 0.3; 
sig3 = 0.3; # volatility 1, 2, 3
rho12 = 0.5; 
rho13 = 0.5;
rho23 = 0.5; # correlation
T = 3.0; # maturity
...
# random number
mat =  np.matrix([[1, rho12, rho13],[rho12, 1, rho23],[rho13, rho23, 1]]); # correlation matrix
chol = np.linalg.cholesky(mat); # cholesky decomposition - lower matrix
z = np.matrix(np.random.normal(size = [3, ns * nStep]));
z = chol * z;
...
for i in xrange(nStep):
    ...
    s3[:] = np.multiply(s3[:], np.exp(drift3 + sigsqdt3*z[2, (ns*i)+tmpIdx]));
    
    # worst perfomer
    wp = np.min([s1, s2, s3], axis = 0);
